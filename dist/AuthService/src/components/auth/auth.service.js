    ...
    login(payload, user) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.passwordsService.comparePassword(payload.password, user.password)) {
                throw new common_1.Unauthorized({ error: common_1.Messages.WRONG_PASSWORD });
            }
            const token = jsonwebtoken_1.sign({ email: user.email, id: user.id }, common_1.config.common.jwtSecret);
            return {
                token,
                user
            };
